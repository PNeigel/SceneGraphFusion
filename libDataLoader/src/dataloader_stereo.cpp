...
DatasetLoader_Stereo::DatasetLoader_Stereo(std::shared_ptr<DatasetDefinitionBase> dataset):
...
bool DatasetLoader_Stereo::IsV2() const {
    return  m_dataset->rotate_pose_img;
}

bool DatasetLoader_Stereo::Retrieve() {
...
void DatasetLoader_Stereo::Reset() {
    frame_index = 0;
}

Eigen::Matrix<float,4,4> DatasetLoader_Stereo::rotation_matrix_Z(const float rot) {
