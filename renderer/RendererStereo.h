...
namespace PSLAM {
    class MeshRendererStereo : public MeshRendererInterface {
    public:
        MeshRendererStereo(int width, int height, const std::string &dataPath, const std::string &sequence, bool toReference):
...
//        float mNearPlane, mFarPlane;
    private:
        ...
        void Init(){
            std::cout << "Inside MeshRendererStereo::Init()" << std::endl;
            ...
            m_scanId = "4acaebcc-6c10-2a2a-858b-29c7e4fb410d";

            if (mb_toRef) {
                auto scan3rLoader = std::make_unique<PSLAM::io::Scan3RLoader>(m_folder + "/3RScan.json");
                std::cout << "Created scan3rLoader" << std::endl;

                if(scan3rLoader->IsRescan(m_scanId)) {
                    std::cout << "inside if IsRescan()" << std::endl;
                    // find ref scan ID
                    ...
                    std::cout << "gt_transformation: " << gt_transformation << std::endl;
            ...
            std::cout << "Loading and setting shaders" << std::endl;
            // Load and set shader
